...
if __name__ == '__main__':
    ...
    train_losses = np.zeros((len(depth_list), epochs))
    val_losses = np.zeros((len(depth_list), epochs))

    for i in range(len(depth_list)):
        configs['model_info']["depths"] = depth_list[i]
        model = build_model(configs['model_info']).to(device)
        optimizer = build_optimizer(model, configs['optimizer_info'])
    ...
    if args.save_loss:
        ...
        with open(f"{model_path}_{op_name}_udepth_loss.npz","wb") as f:
            d = {'file': f, 'train_losses': train_losses, 'val_losses': val_losses,'depth_list':depth_list}
            np.savez(**d)
