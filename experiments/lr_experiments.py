...
from config import build_model,load_config,build_optimizer
...
if __name__ == '__main__':
    ...
    train_losses = np.zeros((len(lr_list),epochs))
    val_losses = np.zeros((len(lr_list),epochs))

    for i in range(len(lr_list)):
        configs['optimizer_info']['lr'] = lr_list[i]
        model = build_model(configs['model_info']).to(device)
        optimizer = build_optimizer(model, configs['optimizer_info'])
    ...
    if args.save_loss:
        ...
        with open(f"{model_path}_{op_name}_loss.npz","wb") as f:
            d = {'file': f, 'train_losses': train_losses, 'val_losses': val_losses,'lr_list':lr_list}
            np.savez(**d)
