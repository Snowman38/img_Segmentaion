...
if __name__ == '__main__':
    ...
    if args.use_val:
        ...
        training_data_loader = DataLoader(dataset=train_dev_sets, num_workers=num_workers, batch_size=batch_size, shuffle=True)
    else:
    ...
    model_list = ["EffUNet","SwinTransformer","SETR"]
    input_c_map = {"EffUNet":3,"SwinTransformer":1,"SETR":1}
    train_losses = np.zeros((len(model_list), epochs))
    val_losses = np.zeros((len(model_list), epochs))

    for i in range(len(model_list)):
        configs['model_info'] = {
            "input_channels": input_c_map[model_list[i]],
            "model_name": model_list[i],
            "model_path": "models/three_models"
        }
    ...
    if args.save_loss:
        ...
        with open(f"{model_path}_{op_name}_loss.npz","wb") as f:
            d = {'file': f, 'train_losses': train_losses, 'val_losses': val_losses,'model_list':model_list}
            np.savez(**d)
