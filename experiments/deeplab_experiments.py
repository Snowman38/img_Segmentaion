...
if __name__ == '__main__':
    ...
    backbone_list = ['resnet','xception','drn','mobilenet']
    train_losses = np.zeros((len(backbone_list), epochs))
    val_losses = np.zeros((len(backbone_list), epochs))

    for i in range(len(backbone_list)):
        configs['model_info']["backbone"] = backbone_list[i]
        model = build_model(configs['model_info']).to(device)
        optimizer = build_optimizer(model, configs['optimizer_info'])
    ...
    if args.save_loss:
        ...
        with open(f"{model_path}_{op_name}_udepth_loss.npz","wb") as f:
            d = {'file': f, 'train_losses': train_losses, 'val_losses': val_losses,'backbone_list':backbone_list}
            np.savez(**d)
