...
if __name__ == '__main__':
    ...
    train_losses = np.zeros((len(b_list),epochs))
    val_losses = np.zeros((len(b_list),epochs))
    ...
    if args.save_loss:
        ...
        with open(f"{model_path}_{op_name}_batch_loss.npz","wb") as f:
            d = {'file': f, 'train_losses': train_losses, 'val_losses': val_losses,'b_list':b_list}
            np.savez(**d)
